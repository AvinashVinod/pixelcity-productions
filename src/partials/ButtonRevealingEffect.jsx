  ...
  return (
    <div className={`relative group cursor-pointer overflow-hidden ${className || 'font-semibold'}`}>
      <div className="relative w-fit">
        <h2
          ref={textRef}
          className={`${idx ? '' : 'small-para'} transition-all duration-300 group-hover:-translate-y-full ${padding}`}
        >
          {text}
        </h2>
      ...
      <h2
        ref={textRef2}
        className={`absolute ${idx ? 'bg-black' : 'small-para'} text-white w-full transition-all duration-300 translate-y-full group-hover:translate-y-0 top-0 left-0 ${padding}`}
      >
        {text}

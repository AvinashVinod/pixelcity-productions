...
const PreWeddingDetailPage = () => {
  ...
  useEffect(() => {
    if (!slug) { setError("No pre-wedding specified"); setLoading(false); return; }

    client.fetch(`*[_type == "prewedding" && slug.current == $slug][0] {
    ...
    .then((data) => {
      if (!data) { setError("Pre-wedding not found"); setLoading(false); return; }
      setDetail(data);
      setGallery(
  ...
  if (!detail) return (
    <section className="pt-25 px-[1rem] md:px-[2rem] pb-20 bg-white min-h-screen flex items-center justify-center">
      <div className="text-center">
        <h2 className="text-3xl font-bold mb-4">Pre-Wedding Not Found</h2>
        <p className="text-gray-600">The pre-wedding story you're looking for doesn't exist.</p>
      </div>
    </section>
  );
        ...
        <div className="flex flex-col md:flex-row gap-10 md:h-dvh bg-[#E3E2DD] p-[1rem] md:p-10 rounded-lg">
          <div className="md:basis-[50%] h-[70vh] md:h-full w-full rounded-lg overflow-hidden">
            <img
              ...
              alt={detail.mainImageAlt || `Pre-wedding story of ${detail.coupleName}`}
...
export default PreWeddingDetailPage;
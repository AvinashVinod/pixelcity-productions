...
const WeddingDetailPage = () => {
  ...
  return (
    <>
      {/*
        ✅ Modal is OUTSIDE pageRef — GSAP applies transform (y: 50 → 0)
        on pageRef which breaks position:fixed inside it.
        Keeping modal outside means it's fixed to the true viewport,
        not the transformed parent — same reason Monochrome works correctly.
      */}
      {selectedImage && (
        <ImageModal
          image={selectedImage}
      ...
      <section ref={pageRef} className="pt-25 px-[1rem] md:px-[2rem] pb-20 bg-white overflow-hidden">
          ...
          <div className="hero-content md:basis-[50%] flex flex-col justify-center gap-10">
            ...
            <div className="flex gap-3">
              ...
              {detail.youtubeVideoId && (
                <a href={`https://www.youtube.com/watch?v=${detail.youtubeVideoId}`} target="_blank" rel="noopener noreferrer"
                  className="text-[2rem] cursor-pointer hover:opacity-70 transition-opacity">
                  <TfiYoutube />
                </a>
              )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 bg-[#E3E2DD] p-[1rem] md:p-10 rounded-lg">
          {gallery.map((item) => (
            <div
              key={item.id}
              className="collage-card group relative w-full aspect-[3/4] bg-zinc-100 overflow-hidden rounded-lg cursor-pointer"
